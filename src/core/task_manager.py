...
class AppSettingsManager:
    ...
    def load_settings(self):
        default_settings = {"sidebar_width": 200, "sidebar_visible": True, "window_size": [1280, 720], "theme": "light"}
        try:
        ...
        try:
            ...
            for index, task_id in enumerate(self.selected_tasks):
                ...
                try:
                    ...
                    # 过滤掉函数不接受的参数，避免TypeError
                    sig = inspect.signature(task_func)
                    valid_params = {k: v for k, v in task_params.items() if k in sig.parameters}
                    success = task_func(self.auto_instance, **valid_params)
