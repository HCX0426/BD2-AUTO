...
class ImageProcessor:
    ...
    def __init__(
        ...
        if not logger:
            raise ValueError("[ImageProcessor] 初始化失败：logger不能为空（必须传入有效日志实例）")
        if not isinstance(original_base_res, (tuple, list)) or len(original_base_res) != 2:
            raise ValueError(f"[ImageProcessor] 初始化失败：original_base_res必须是二元组（当前：{original_base_res}）")
        if original_base_res[0] <= 0 or original_base_res[1] <= 0:
            raise ValueError(f"[ImageProcessor] 初始化失败：original_base_res必须为正整数（当前：{original_base_res}）")
        if not isinstance(coord_transformer, CoordinateTransformer):
            raise ValueError("[ImageProcessor] 初始化失败：coord_transformer必须是CoordinateTransformer实例")
        if not isinstance(display_context, RuntimeDisplayContext):
            raise ValueError("[ImageProcessor] 初始化失败：display_context必须是RuntimeDisplayContext实例")
        ...
        self.logger.info(
            f"[ImageProcessor] 初始化完成 | 加载模板数: {len(self.templates)} | "
    ...
    def load_template(self, template_name: str, template_path: Optional[str] = None) -> bool:
        try:
            ...
            if not os.path.exists(template_path):
                self.logger.error(f"[ImageProcessor] 模板不存在: {template_path}")
                return False

            template_color = cv2.imread(template_path, cv2.IMREAD_COLOR)
            if template_color is None:
                self.logger.error(f"[ImageProcessor] 读取模板失败: {template_path}")
                return False

            self.templates[template_name] = template_color
            self.logger.debug(f"[ImageProcessor] 加载模板成功: {template_name} | 原始尺寸: {template_color.shape[:2]}")
            return True
        except Exception as e:
            self.logger.error(f"[ImageProcessor] 加载模板异常: {str(e)}", exc_info=True)
            return False

    def load_all_templates(self) -> None:
        if not os.path.isdir(self.template_dir):
            self.logger.warning(f"[ImageProcessor] 模板目录不存在: {self.template_dir}")
            return
    ...
    def get_template(self, template_name: str) -> Optional[np.ndarray]:
        ...
        if template is None:
            self.logger.warning(f"[ImageProcessor] 模板未找到，尝试重新加载: {template_name}")
    ...
    def match_template(
        ...
        try:
            ...
            self.logger.debug(
                f"[ImageProcessor] 上下文全屏状态: {is_fullscreen} | ROI: {roi} | 阈值: {threshold} | "
                f"客户区逻辑尺寸: {ctx.client_logical_res} | 屏幕物理尺寸: {ctx.screen_physical_res}"
            )

            if image is None or image.size == 0:
                self.logger.error("[ImageProcessor] 模板匹配失败：输入图像为空或无效")
                return None
            ...
            if isinstance(template, str):
                ...
                if template_bgr is None:
                    self.logger.error(f"[ImageProcessor] 模板匹配失败：模板「{template}」不存在或加载失败")
                    return None
                template_name = template
            else:
                if not isinstance(template, np.ndarray) or template.size == 0:
                    self.logger.error("[ImageProcessor] 模板匹配失败：自定义模板为无效numpy数组")
                    return None
                template_bgr = template
                template_name = "custom_template"
            template_orig_h, template_orig_w = template_bgr.shape[:2]
            self.logger.debug(
                f"[ImageProcessor] 开始模板匹配 | 模板: {template_name} | 原始尺寸: (宽={template_orig_w}, 高={template_orig_h}) | "
            ...
            if roi:
                try:
                    ...
                    if is_fullscreen:
                        ...
                        self.logger.debug(
                            f"[ImageProcessor] 全屏模式ROI处理 | 原始物理ROI: {roi} → 调整后: ({rx_phys}, {ry_phys}, {rw_phys}, {rh_phys}) | "
                        ...
                        self.logger.debug(
                            f"[ImageProcessor] 窗口模式ROI处理 | 原始逻辑ROI: {roi} → 调整后逻辑ROI: ({rx_log}, {ry_log}, {rw_log}, {rh_log}) → "
                    ...
                    self.logger.debug(
                        f"[ImageProcessor] ROI裁剪完成 | 子图尺寸: (宽={cropped_phys_w}, 高={cropped_phys_h}) | "
                    ...
                    if config.get("debug", False):
                        ...
                        self.logger.debug(f"[ImageProcessor] ROI子图已保存至: {crop_save_path}")

                    processed_roi = (rx, ry, rw, rh)

                except ValueError as e:
                    self.logger.warning(f"[ImageProcessor] ROI处理失败：{str(e)}，自动切换为全图匹配")
                    cropped_image = orig_image
                    processed_roi = None
                    roi_offset_phys = (0, 0)
            ...
            if processed_roi:
                ...
                self.logger.debug(
                    f"[ImageProcessor] 模板缩放比例计算（有ROI）| 子图尺寸: (宽={cropped_phys_w}, 高={cropped_phys_h}) | "
                ...
                scale_ratio = min(scale_ratio_w, scale_ratio_h)
                self.logger.debug(
                    f"[ImageProcessor] 模板缩放比例计算（无ROI）| 子图尺寸: (宽={cropped_phys_w}, 高={cropped_phys_h}) | "
            ...
            if scaled_w <= 0 or scaled_h <= 0:
                self.logger.error(
                    f"[ImageProcessor] 模板缩放失败 | 缩放后尺寸无效: (宽={scaled_w}, 高={scaled_h}) | "
            ...
            self.logger.debug(
                f"[ImageProcessor] 模板缩放完成 | 原始尺寸: (宽={template_orig_w}, 高={template_orig_h}) → "
            ...
            if (template_gray.shape[0] > cropped_gray.shape[0]) or (template_gray.shape[1] > cropped_gray.shape[1]):
                self.logger.error(
                    f"[ImageProcessor] 模板匹配失败：模板尺寸超过子图尺寸 | "
            ...
            match_score = max_val
            self.logger.debug(
                f"[ImageProcessor] 模板匹配完成 | 最高匹配分数: {match_score:.4f} | 阈值: {threshold} | "
                f"匹配位置（子图内）: {max_loc}"
            )

            if match_score < threshold:
                self.logger.debug(
                    f"[ImageProcessor] 模板匹配失败：分数不足 | 模板: {template_name} | 分数: {match_score:.4f} < 阈值: {threshold}"
                )
                if config.get("debug", False):
                    safe_template_name = template_name.replace("/", "_").replace("\\", "_")
                    ...
                    self.logger.debug(f"[ImageProcessor] 匹配失败调试图已保存至: {save_path}")
                return None
            ...
            self.logger.debug(
                f"[ImageProcessor] 坐标映射完成 | 子图内坐标: ({match_x_sub}, {match_y_sub}) → "
            ...
            if config.get("debug", False):
                ...
                cv2.imwrite(save_path, debug_img)
                self.logger.debug(f"[ImageProcessor] 匹配成功调试图已保存至: {save_path}")

            self.logger.info(
                f"[ImageProcessor] 模板匹配成功 | 模板: {template_name} | 逻辑坐标: {final_bbox_log} | "
            ...
            self.logger.error(f"[ImageProcessor] 模板匹配异常 | 模板: {template_name} | 错误: {str(e)}", exc_info=True)
            return None

    def get_center(self, rect: Union[Tuple[int, int, int, int], np.ndarray]) -> Tuple[int, int]:
        if rect is None:
            self.logger.warning("[ImageProcessor] 计算中心失败：矩形为None")
            return (0, 0)
        if isinstance(rect, np.ndarray):
            rect = tuple(rect.tolist())
        if not isinstance(rect, (tuple, list)) or len(rect) != 4:
            self.logger.warning(f"[ImageProcessor] 计算中心失败：无效矩形 {rect}")
            return (0, 0)
        try:
            ...
            self.logger.warning(f"[ImageProcessor] 计算中心失败：矩形值错误 {rect}")
            return (0, 0)